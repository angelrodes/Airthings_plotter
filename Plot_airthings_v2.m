...
valid=~isnan(Rn6h);
plot(posix_time(valid),Rn6h(valid),'-k','LineWidth',2)
...
if mylaptop>0
    ...
    valid=~isnan(Rn6h);
    plot(posix_time(valid),Rn6h(valid),'-b','LineWidth',2)
    
    legend('Reported 24h average','6h moving average','AutoUpdate','off','Location','northwest')
    
    for alpha=linspace(-1,1,100) % uncertainty
        plot(posix_time(valid),Rn6h(valid)+Rn6h_uncert(valid)*alpha,'-','Color',[0.7 0.7 0.7],'LineWidth',2)
    end
