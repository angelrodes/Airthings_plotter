...
for n=find(~isnan(Rn))'
    ...
    previous_24h_data=Rn2(max(1,Rn_index(max(1,step-N))):Rn_index(max(1,step-1)));
    previous_24h_data=previous_24h_data(~isnan(previous_24h_data));
    step_data=Rn2(max(1,Rn_index(max(1,step-1))+1):Rn_index(step));
    step_value=(...
...
y=Rn2';
yy=Rn2';
span=6*round(median(diff(find(~isnan(Rn)))));
